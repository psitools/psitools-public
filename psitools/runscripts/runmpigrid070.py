...
arglist = []
for k in ks:
    arglist.append({'tsint':tsint, 'epstot':epstot, 'sigma':sigma, 'peak':peak, 'kx':k[0], 'kz':k[1], 'refine':refine})

def clean_results():
    for args in arglist:
        ait = streamingtools.AitkenConvergerLogNormal(**args)
        try:
...
def collect_results():
    ...
    for ix, kx in enumerate(kxaxis):
        for iz, kz in enumerate(kzaxis):
            ait = streamingtools.AitkenConvergerLogNormal(**arglist[flatindex])
            try:
    ...
    with h5py.File(database_hdf5,'w') as h5f:
        ...
        grp.attrs['distribution'] = 'lognormal'
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'compute':
        streamingtools.MpiScheduler(arglist, wall_start, wall_limit_total, disttype = 'lognormal')
