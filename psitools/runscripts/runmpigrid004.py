...
reruns_file = batchname+'_rerun.txt'
...
def collect_results():
    # Read in the outputs
    fastesteigens = np.empty([refine] + list(kxgrid.shape), dtype=np.complex)
    aitkengrowthrates = np.empty([refine-2] + list(kxgrid.shape))
    aitkenoscillate = np.empty([refine-2] + list(kxgrid.shape))
    computetimes = np.empty([refine] + list(kxgrid.shape))
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'compute':
        ...
        with open(rerunsfile,'r') as rerunfile:
