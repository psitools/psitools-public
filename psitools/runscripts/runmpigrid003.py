...
def collect_results():
    ...
    #aitkengrowthrates2 = np.zeros([refine-4] + list(kxgrid.shape))
    #aitkenoscillate2 = np.zeros([refine-4] + list(kxgrid.shape))
        ...
        for iz, kz in enumerate(kzaxis):
            ...
            try:
                with open(ait.picklename,'rb') as picklefile:
                    ...
                    #aitkengrowthrates2[:,ix,iz] = ait.aitkengrowthrates2
                    #aitkenoscillate2[:,ix,iz] = ait.aitkenoscillate2
                    computetimes[:,ix,iz] = ait.computetimes
                del(ait)
            except FileNotFoundError as fe:
                #print('File not found ',ait.picklename,' index ',flatindex)
                #print(' ',arglist[flatindex])
                print(flatindex)
            flatindex += 1
    ...
    with h5py.File(database_hdf5,'w') as h5f:
        ...
        #dset = grp.create_dataset('aitkengrowthrates2', data=aitkengrowthrates2)    
        #dset = grp.create_dataset('aitkenoscillate2', data=aitkenoscillate2)    
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'compute':
        ...
        with open('run3_rerun.txt','r') as rerunfile:
